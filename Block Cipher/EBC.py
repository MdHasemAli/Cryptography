...
def ecb_encrypt(message, key, block_size):
    ...
    encrypted = [xor_bits(block, key) for block in blocks]
    return ''.join(encrypted), original_len

def ecb_decrypt(cipher_binary, key, block_size, original_len):
    blocks = split_into_blocks(cipher_binary, block_size)
    decrypted = [xor_bits(block, key) for block in blocks]
    binary = ''.join(decrypted)
...
if __name__ == "__main__":
    ...
    cipher, length = ecb_encrypt(message, key, block_size)
    print("Encrypted Text:", binary_to_text(pad_binary(cipher, 8)))
    print("Decrypted Text:", ecb_decrypt(cipher, key, block_size, length))
